...
def play(args):
    ...
    if RECORD_FRAMES and not env.env.base_env.headless:
        try:
            ...
            # 设定相机初始位置与朝向
            cam_pos = gymapi.Vec3(3.0, 0.0, 1.5)  # 更高更远的视角，以便更好地观察整个场景
            ...
            try:
                ...
                if camera_handle is None:
                    ...
                    try:
                        ...
                        # 设定相机初始位置与朝向
                        cam_pos = gymapi.Vec3(3.0, 0.0, 1.5)  # 更高更远的视角
                        cam_target = gymapi.Vec3(0.0, 0.0, 0.4)
                        env.env.base_env.gym.set_camera_location(camera_handle, env.env.base_env.envs[0], cam_pos, cam_target)
                ...
                if i % 5 == 0:  # 每5帧更新一次相机位置，减少计算量
                    try:
                        ...
                        # 调整相机距离，确保可以看到目标和障碍物
                        cam_pos = gymapi.Vec3(
                            float(target_pos[0]) - 3.0,  # 更远的距离
                            float(target_pos[1]) + 0.0, 
                            float(target_pos[2]) + 1.5   # 更高的视角
                        )
                        env.env.base_env.gym.set_camera_location(camera_handle, env.env.base_env.envs[0], cam_pos, cam_target)
